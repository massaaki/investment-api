...
export class DbCreateStockMarketIndexDailyVariation implements ICreateStockMarketIndexDailyVariationIndex {
  ...
  constructor(
    ...
    private readonly webRequestStockInformations: IWebRequestStockInformations
  ) {}

  async create({ code }: CreateStockMarketIndexDailyVariationRequestProps): Promise<IStockMarketIndexDailyVariation> {
    ...
    //STEP 3 - Get History values from alphavantage api
    // console.log('code..:', code);
    // const response = await this.webRequestStockInformations.getStocksInformations({ code });
